...
--****************************** TEXTBOX ***********************
	elseif(obj.type == TTEXTBOX) then
		...
		if(opt4 ~= nil) then obj.image = opt4 
		else obj.image = false end
--****************************** LINK ***********************
...
luaWidget.del = function(parent, objet)
	...
	if(parent ~= nil) then
		if(objet ~=nil) then
			...
			if(luaWidget.tabobj[parent][objet].image ~= 0 and luaWidget.tabobj[parent][objet].type ~= TTEXTBOX) then
				Image.destroy(luaWidget.tabobj[parent][objet].image)
			end
			luaWidget.tabobj[parent][objet].type = 0
		else
--			for j =1,#luaWidget.tabobj[parent] do
--				luaWidget.del(luaWidget.tabobj[parent],j)
--			end
		end
	else
		TBUTTON = nil
...
--******************** ELEMENT Pour LISTVIEW ********************
				elseif(luaWidget.tabobj[parent][obj].type == TLISTVIEW) then
					buff.text = value[i]
					table.insert(luaWidget.tabobj[parent][obj].element,buff)
--******************** ELEMENT Pour TEXTBOX multiline ********************
				elseif(luaWidget.tabobj[parent][obj].type == TTEXTBOX) then
					buff.text = value[i]
					table.insert(luaWidget.tabobj[parent][obj].element,buff)
...
--******************** MIN ********************
		elseif(opt == "min") then
			if(luaWidget.tabobj[parent][obj].type == TTEXTBOX) then --***** TEXT BOX
				-- on fait rien
			else
				luaWidget.tabobj[parent][obj].min = value
				if(luaWidget.tabobj[parent][obj].pos < value) then 
...
--******************** POS ********************
		elseif(opt == "pos") then
			if(luaWidget.tabobj[parent][obj].type == TTEXTBOX) then --***** TEXT BOX
				if(value > #luaWidget.tabobj[parent][obj].element) then
					value = #luaWidget.tabobj[parent][obj].element
				end
				if(value < 1) then value = 1 end
				if(luaWidget.tabobj[parent][obj].element[value] ~= nil) then
				if(luaWidget.tabobj[parent][obj].inc > string.len(luaWidget.tabobj[parent][obj].element[value])) then 
					luaWidget.tabobj[parent][obj].inc = string.len(luaWidget.tabobj[parent][obj].element[value])+1
				end
				end
			else
				if(value > luaWidget.tabobj[parent][obj].max) then
					value = luaWidget.tabobj[parent][obj].max
				end
				if(value < luaWidget.tabobj[parent][obj].min) then
					value = luaWidget.tabobj[parent][obj].min
				end
			end
			luaWidget.tabobj[parent][obj].pos = value
--******************** INC ********************
		elseif(opt == "inc") then
			if(luaWidget.tabobj[parent][obj].type == TTEXTBOX) then --***** TEXT BOX
				local pos = luaWidget.tabobj[parent][obj].pos
				local max = 1
				if(luaWidget.tabobj[parent][obj].element[pos] ~= nil) then
					max = string.len(luaWidget.tabobj[parent][obj].element[pos])+1
				end
				if(value < 1) then value = 1 end
			...
			luaWidget.tabobj[parent][obj].inc = value
--******************** MULTILINE ********************
...
--******************** HEIGHT AND TYPE= TTEXTBOX ********************
		elseif(opt == "height" and luaWidget.tabobj[parent][obj].type == TTEXTBOX) then
			luaWidget.tabobj[parent][obj].height = value
...
luaWidget.addElement = function(parent,obj,element,pos)
	if(pos == nil) then pos = #luaWidget.tabobj[parent][obj].element+1 end
--********************* LISTVIEW **********************
	if(luaWidget.tabobj[parent][obj].type == TLISTVIEW) then
		local buff = {}
		buff.text = element
		table.insert(luaWidget.tabobj[parent][obj].element,pos,buff)
		luaWidget.tabobj[parent][obj].max = luaWidget.tabobj[parent][obj].max+1
	end
--********************* TABLEVIEW **********************
	if(luaWidget.tabobj[parent][obj].type == TTABLEVIEW) then
		table.insert(luaWidget.tabobj[parent][obj].element,pos,element)
		luaWidget.tabobj[parent][obj].max = #luaWidget.tabobj[parent][obj].element -1
		luaWidget.tabobj[parent][obj].pos = #luaWidget.tabobj[parent][obj].element -1
	end
--********************* TEXTBOX **********************
	if(luaWidget.tabobj[parent][obj].type == TTEXTBOX) then
		table.insert(luaWidget.tabobj[parent][obj].element,pos,element)
		luaWidget.tabobj[parent][obj].pos = pos
		luaWidget.tabobj[parent][obj].inc = string.len(element)+1
	end
end

luaWidget.delElement = function(parent,obj,pos)
	if(pos == nil) then pos = 1 end
--********************* LISTVIEW **********************
	if(luaWidget.tabobj[parent][obj].type == TLISTVIEW) then
		table.remove(luaWidget.tabobj[parent][obj].element,pos)
		luaWidget.tabobj[parent][obj].max = luaWidget.tabobj[parent][obj].max-1
	end
--********************* TABLEVIEW **********************
	if(luaWidget.tabobj[parent][obj].type == TTABLEVIEW) then
...
--********************* TEXTBOX **********************
	if(luaWidget.tabobj[parent][obj].type == TTEXTBOX) then
		table.remove(luaWidget.tabobj[parent][obj].element,pos)
		luaWidget.tabobj[parent][obj].pos = luaWidget.tabobj[parent][obj].pos -1
		if(luaWidget.tabobj[parent][obj].pos == 0) then
			luaWidget.tabobj[parent][obj].pos = 1 
			luaWidget.tabobj[parent][obj].inc = 1
		else
			luaWidget.tabobj[parent][obj].inc = string.len(luaWidget.tabobj[parent][obj].element[luaWidget.tabobj[parent][obj].pos])+1
		end
		if(luaWidget.tabobj[parent][obj].min > 1) then
			luaWidget.tabobj[parent][obj].min = luaWidget.tabobj[parent][obj].min -1
		end
	end
end

luaWidget.modElement = function(parent,obj,pos,value)
	if(pos == nil) then pos = 1 end
	if(value ~=nil) then
--********************* TEXTBOX **********************
		if(luaWidget.tabobj[parent][obj].type == TTEXTBOX) then
			luaWidget.tabobj[parent][obj].element[pos] = value
			--luaWidget.tabobj[parent][obj].inc = string.len(value)+1
		end
	end
end

luaWidget.getElement = function(parent,obj,pos)
	if(pos == nil) then pos = 1 end
--********************* TEXTBOX **********************
	if(luaWidget.tabobj[parent][obj].type == TTEXTBOX) then
		return luaWidget.tabobj[parent][obj].element[pos]
	end
end

luaWidget.getNbElement = function(parent,obj)
	return #luaWidget.tabobj[parent][obj].element
end
...
-- ************** BOUTON *****************
			if(obj.type == TBUTTON) then
				...
				if(obj.active) then screen.print(scr,xx+add,yy+add,buff,obj.cText)
				else screen.print(scr,xx+add,yy+add,buff,obj.cFond) end
-- ************** TEXTBOX *****************
			elseif(obj.type == TTEXTBOX) then
				screen.drawFillRect(scr, obj.x, obj.y, obj.x+obj.width, obj.y+obj.height, obj.cFond)
				...
				local maxcavi = math.floor((obj.width-6)/6)
				if(obj.max == 0) then
					...
					local x,j,k
					posy = obj.y+2
					if(#obj.element>0) then
						if(obj.pos > (obj.min-1)+obj.max) then obj.min = (obj.pos-obj.max)+1 end
						if(obj.pos < obj.min) then obj.min = obj.pos end
						for i= 0, obj.max-1 do
							if(obj.element[obj.min+i] ~= nil) then
								buft = obj.element[obj.min+i]
								else buft = "" end
							x = (obj.inc - maxcavi)+1
							if x<1 then x = 1 end
							table.insert(buff,string.sub(buft,x,maxcavi+(x-1)))
						end
					else
						x = 1
						for i= 1, obj.max do
							buft = obj.text
							j,k = string.find(buft,"\\n",x)
							if(j==nil) then
								table.insert(buff,string.sub(buft,x,maxcavi+(x-1)))
								x = x + maxcavi
							else
								if(j<maxcavi+(x-1)) then
									table.insert(buff,string.sub(buft,x,j-1))
									x = k+1
								else
									table.insert(buff,string.sub(buft,x,maxcavi+(x-1)))
									x = x + maxcavi
				...
				if(obj.image) then
					local x = obj.inc
					local y = obj.pos-obj.min+1
					if(x>maxcavi) then x = maxcavi end
					if(y>obj.max) then y = obj.max end
					posx = (obj.x+2+((x-1)*6))+1
					screen.drawLine(scr,posx,posy+((y*10)-2),posx+5,posy+((y*10)-2),obj.cFleche)
				end
-- ************** CHECKBOX *****************
			elseif(obj.type == TCHECKBOX) then
				local xx = obj.x
